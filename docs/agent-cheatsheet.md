...
- `pmtl-runbook-cms-runtime-errors` (Việt: PMTL runbook cms runtime errors): [ĐÃ LỖI THỜI - DEPRECATED] Kỹ năng tham chiếu tới cấu trúc CMS cũ (nay backend la NestJS). KHÔNG sử dụng skill này cho các quy trình sửa lỗi mới.
...
- `pmtl-scaffold-payload-collection` (Việt: PMTL scaffold payload collection): [ĐÃ LỖI THỜI - DEPRECATED] Dùng cho scaffold legacy CMS collection (khong con hoat dong trong repo). Cấu trúc hiện tại của dự án dùng NestJS thay cho backend. KHÔNG sử dụng.
