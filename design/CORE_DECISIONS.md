...
### Context
Repo hiện có cache/queue kiểu Redis-compatible, worker (tiến trình xử lý nền) và Meilisearch.
...
- `Valkey` (`Redis-compatible`) chỉ giữ cache, queue (hàng đợi xử lý) state, coordination state.
...
- Không phát business event quan trọng bằng Redis/Valkey pub/sub thuần hoặc fire-and-forget direct call từ request path.
- Execution queue vẫn được phép tồn tại, nhưng nó là tầng thực thi sau outbox chứ không phải transactional handoff gốc.
- Preferred in-memory store cho current stack là `Valkey`; execution queue implementation ưu tiên là `BullMQ` thay vì hand-rolled Redis queue semantics.
...
### Context
`Valkey` (`Redis-compatible`) hiện được phép dùng cho cache nhưng không được phép chứa canonical business truth.
...
## Decision 15. App-layer rate limit dùng Valkey hoặc Redis-compatible store là bắt buộc cho public write/auth/search surfaces

### Context
Public auth, search, submit form, và upload file đều là bề mặt dễ bị abuse hoặc accidental burst trên 1 VPS.

### Decision
- Rate limit phải đặt ở app layer và dùng `Valkey` hoặc Redis-compatible store làm counter store.
- Tối thiểu áp cho:
  - login
  - register
  - forgot password
...
- Khi `Valkey` hoặc Redis-compatible store lỗi, cần degrade policy rõ thay vì bỏ trống guard.
