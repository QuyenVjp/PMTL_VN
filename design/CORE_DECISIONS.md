...
- Hard delete phải đi qua cleanup contract của owner module; nếu chưa có cleanup path rõ, dùng soft delete hoặc archive.
...
- Search sync guarantee hiện tại là `at-least-once async projection`, không phải exactly-once synchronous mirror.
