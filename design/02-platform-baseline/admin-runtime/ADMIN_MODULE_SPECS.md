...
> **Shell architecture**: `design/02-platform-baseline/admin-runtime/ADMIN_ARCHITECTURE.md`
...
> **Query keys + invalidation canon**: `design/04-execution-overlay/admin/ADMIN_PAGE_API_MAPPING.md`
...
**Query invalidation**: After publish/unpublish → follow `design/04-execution-overlay/admin/ADMIN_PAGE_API_MAPPING.md` query-key canon for `admin-posts` list/detail + dashboard widget dependencies when affected.
...
Note: chưa claim dedicated `GET /api/content/media/:publicId` canon cho detail modal; nếu cần route riêng thì phải mở thêm row trong `design/04-execution-overlay/api/API_ROUTE_INVENTORY.md` + DTO owner trong `design/04-execution-overlay/api/API_DTO_SHAPE_PLAN.md` trước khi scaffold.
...
When flag = true: Full push job management (see `design/02-platform-baseline/optional-scale/PUSH_NOTIFICATION_ARCHITECTURE.md`)
...
See `design/02-platform-baseline/api-runtime/HEALTH_CONTRACT.md` for full spec.
...
See `design/03-domains/vows-merit/REFERENCES/ASSISTED_ENTRY_WORKFLOW.md` for full workflow details.
