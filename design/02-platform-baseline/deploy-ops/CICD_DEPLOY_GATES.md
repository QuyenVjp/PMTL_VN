...
> **Deploy runbook**: `design/02-platform-baseline/deploy-ops/DEPLOY_RUNBOOK.md` — manual deploy procedure
> **Testing strategy**: `design/02-platform-baseline/deploy-ops/TESTING_STRATEGY.md`
> **Secret management**: `design/02-platform-baseline/security-runtime/SECRET_MANAGEMENT.md`
...
```
**Gate**: Fail → block merge, block deploy.
**Coverage target**: ≥ 70% statement coverage (see `design/02-platform-baseline/deploy-ops/TESTING_STRATEGY.md`)
...
Deploy outputs sau khi chạy phải được ghi về deploy record canon theo `design/04-execution-overlay/repo/DEPLOY_RECORD_TEMPLATE.md`.
...
- Backup artifact naming nên khớp contract trong `design/02-platform-baseline/deploy-ops/BACKUP_RESTORE.md`
...
- artifact pinning rõ
- restore/rollback rehearsal record trong `design/04-execution-overlay/repo/RESTORE_DRILL_LOG.md`
- post-rollback health verification
- post-rollback sample canonical read verification

## Deploy record output rule

Pipeline hoặc operator phải map tối thiểu các output sau vào `design/04-execution-overlay/repo/DEPLOY_RECORD_TEMPLATE.md`:
