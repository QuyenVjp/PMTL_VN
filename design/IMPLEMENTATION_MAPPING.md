...
|---|---|---|---|
| `NestJS auth` là auth authority (quyền lực xác thực) duy nhất | `apps/api/src/modules/auth/*` | required before launch | login/refresh/logout/reset routes (đường dẫn đăng nhập/làm mới/đăng xuất/thiết lập lại) + guards (bộ canh phòng) + token/session persistence (lưu trữ token/phiên) |
