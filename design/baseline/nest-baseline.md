...
- env contract chuẩn của PMTL vẫn do `tracking/env-inventory.md` + `common/config/config.schemas.ts` owner, không do Nest docs sample tự quyết
...
- built-in `HttpException` hoặc custom exception kế thừa từ nó vẫn phải quy về `tracking/error-code-registry.md` nếu route đã có owner code
