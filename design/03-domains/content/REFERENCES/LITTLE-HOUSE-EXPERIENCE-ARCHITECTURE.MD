...
> - `design/03-domains/engagement/SCHEMA_PLAN.dbml`
>
...
- `design/03-domains/content/REFERENCES/PRACTICE-UI-CHECKLISTS.MD`
  - đã có stepper/checklist gợi ý cho recitation và burning
- `design/03-domains/engagement/*`
...
Từ `design/03-domains/engagement/SCHEMA_PLAN.dbml`, nên chuẩn bị mở rộng `ngoi_nha_nho_sheets` bằng các field sau:
...
- `design/02-platform-baseline/web-runtime/seo-geo/STRATEGY.md` — chiến lược SEO/GEO toàn site
- `design/03-domains/content/REFERENCES/LITTLE_HOUSE_SEO.md` — keyword research, target pages, Featured Snippet strategy
- `design/02-platform-baseline/web-runtime/seo-geo/STRUCTURED_DATA.md` — HowTo / FAQPage / BreadcrumbList / ItemList schemas
- `design/02-platform-baseline/web-runtime/seo-geo/GEO_CITATION_STRATEGY.md` — để ChatGPT, Gemini, Perplexity trích dẫn PMTL_VN
- `design/02-platform-baseline/web-runtime/seo-geo/CONTENT_CLUSTER_MAP.md` — pillar/spoke cho 6 clusters
...
1. ✅ `design/03-domains/content/REFERENCES/LITTLE-HOUSE-CONTENT-INVENTORY.MD` — đã tạo, canonical content 13→5
2. ✅ `design/03-domains/content/REFERENCES/LITTLE-HOUSE-TECH-FEATURES.MD` — đã tạo, 8 tech features spec
3. ✅ `design/02-platform-baseline/web-runtime/seo-geo/` — đã tạo đầy đủ 5 files SEO/GEO
4. ✅ `design/04-execution-overlay/web/PAGE_INVENTORY.md` — đã cập nhật routes Little House (1.4, 1.4a, 1.4b)
5. ✅ `design/03-domains/search/REFERENCES/UNIFIED_INDEX_MAPPING.md` — đã thêm `little_house_guide` + `little_house_faq`
6. ✅ `design/03-domains/engagement/SCHEMA_PLAN.dbml` — đã thêm 9 fields + 3 enums mới

**Đã làm thêm sau đó**:
- ✅ Scaffold content contracts cho `GET /content/little-house/guide-map` và các endpoints trong §8
- ✅ Tạo `design/03-domains/content/USE_CASES/publish-little-house-guide.md`
- ✅ Cập nhật `design/04-execution-overlay/api/API_ROUTE_INVENTORY.md`

**Còn lại nếu đi tiếp sang code thật**:
- rà tiếp `design/04-execution-overlay/data/PRISMA_SCHEMA_PLAN.md` để merge đầy đủ các enum mở rộng của `04-engagement` khi schema runtime được khóa
