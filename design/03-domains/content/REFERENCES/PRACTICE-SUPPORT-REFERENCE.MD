...
- Pipeline local hiện đã xuất raw artifact vào:
  - `design/03-domains/content/practice-pdf-extracts/README.md`
  - `design/03-domains/content/practice-pdf-extracts/manifest.json`
- Mỗi PDF có:
  - `document.md`
  - `document.json`
...
- File đọc nhanh thêm:
  ...
  - `design/03-domains/wisdom-qa/REFERENCES/AFFINITY-WITH-BUDDHISM-NOTES.md`
...
> Xem sơ đồ nhanh ở `design/03-domains/content/STATES/PRACTICE_SUPPORT_FLOWS.mmd`.
...
- UI gợi ý:
  ...
  - support item như `Công Đức Bảo Sơn Thần Chú` chỉ hiện như optional companion card khi có ngữ cảnh phù hợp, không auto-nhét thành bài bắt buộc cho mọi người
...
- với các bài hỗ trợ kiểu `Công Đức Bảo Sơn Thần Chú`:
  - không dựng `merit wallet`, `merit balance`, hay `% chuyển công đức` thành computed product field
  - nếu user cần lưu ngữ cảnh, chỉ nên là self-note như:
    - việc thiện vừa làm
    - ngày làm
    - đang cầu cho bản thân hay người thân
  - không coi mọi việc thiện quá lâu trong quá khứ là còn nguyên để chuyển
  - không biến app thành công cụ khẳng định “đã chuyển thành công đức thành công”
...
- ưu tiên rà lại:
  ...
  - wording support của `Công Đức Bảo Sơn Thần Chú`, nhất là distinction:
    - việc thiện gần đây có thể chuyển thành công đức
    - việc thiện quá lâu có thể đã thành `phúc đức`

## File nên đọc tiếp theo

- nếu muốn nhập dữ liệu vào `chantItems`:
  - đọc `design/03-domains/content/REFERENCES/CHANT-ITEMS-CATALOG.MD`
- nếu muốn làm UI stepper/checklist:
  - đọc `design/03-domains/content/REFERENCES/PRACTICE-UI-CHECKLISTS.MD`
- nếu muốn làm riêng flow `Ngôi Nhà Nhỏ`:
  - đọc `design/03-domains/content/REFERENCES/LITTLE-HOUSE-SPEC.MD`
