...
| upload hardening (thắt chặt bảo mật tải lên) | upload controller/service + MIME sniffing + allowlist + delete auth | required before launch | Storage service exists but MIME sniffing + allowlist validation + delete authorization guard still need verification. Audit on upload exists via content.create path. |
...
| backup + restore drill (sao lưu + diễn tập phục hồi) | runbook + restore drill record | required before launch | [restore-drill-log.md](./RESTORE_DRILL_LOG.md) có evidence (bằng chứng) thật |
...
| [upload-media-asset.md](../../03-domains/content/USE_CASES/upload-media-asset.md) | upload controller + media service + storage adapter + media_assets table + asset status handling | required before launch | Storage abstraction implemented. Upload hardening (MIME sniffing + allowlist) still needs verification. |
