...
| backup + restore drill (sao lưu + diễn tập phục hồi) | runbook + restore drill record | **implemented** (2026-03-27) | `infra/scripts/backup-db.sh`: `pg_dump -Fc` with retention policy. `infra/scripts/restore-db.sh`: `pg_restore --clean --if-exists` with 4-step verification. Drill #1 executed 2026-03-27 on dev environment: backup 32K, restore `pass`, 9 tables verified. Evidence at [RESTORE_DRILL_LOG.md](./RESTORE_DRILL_LOG.md). Production drill pending before first public launch. |
