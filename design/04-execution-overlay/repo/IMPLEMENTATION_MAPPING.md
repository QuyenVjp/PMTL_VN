...
| `Meilisearch` | `apps/api/src/modules/search/adapters/meilisearch.adapter.ts`; feature flag: `search.meilisearch.enabled` | planned | SQL search p95 vượt SLO public search đã chốt trong `design/02-platform-baseline/deploy-ops/SLA_SLO.md` hoặc multi-type search requirement xuất hiện rõ | `design/02-platform-baseline/optional-scale/MEILISEARCH_ARCHITECTURE.md` |
