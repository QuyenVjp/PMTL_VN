...
- API group có thể là tiếng Anh dưới `/api/admin/*`; mapping canon nằm ở `design/04-execution-overlay/admin/ADMIN_PAGE_API_MAPPING.md`
...
- `users-admin`, `sessions-admin`, `wisdom-baihoa`, `assisted-entry-admin` phải bám chính xác canon route mới thêm trong `design/04-execution-overlay/api/API_ROUTE_INVENTORY.md`, không được scaffold theo API deps cũ trong audit notes.
