...
### 2. **pmtl-redis** — Cache + Queue + Pub/Sub
...
- Worker nhấc job ra xử lý async
...
User publish post
→ save to Postgres
→ push job to Redis Queue: {type: 'index_post', postId: 123}
...
- Queue lỗi → async job không được xử lý (worker đợi)
...
User publish post (Postgres)
    ↓
Trigger job → Redis Queue: {type: 'index_post', data: {...}}
    ↓
Worker pick job
    ↓
...
- Fallback: can query Postgres full-text (chậm hơn)

---

### 5. **pmtl-prometheus** — Metrics Collection

**Chức năng**: Đi "hỏi" metrics từ các service, lưu lịch sử theo thời gian
...
- App custom: queue length, job duration, cache age
...
| Queue           | job queue length, job duration, failed count         |
...
- Trigger auto-remediation (restart service, scale up)
...
Log to alert history DB
    ↓
Optionally trigger action:
  - restart service via webhook
...
- systemd service: 1 process per host (Docker Compose)
...
| Queue (Bull) length      | > 1000             | Add worker instance                          |
| Meilisearch index stale  | > 1 hour           | Check worker, re-trigger reindex             |
...
| Worker job duration  | Identify slow job type  |
...
| Redis         | Cache/queue lỗi          | Slow API, async job backlog             | Restart (< 1 min)                                      |
...
| Alertmanager  | Alert not routed         | Team doesn't know alert                 | Restart, Prometheus queue alert timeout 5 min (~6 min) |
...
- PostgreSQL managed service hoặc self-hosted
- Redis managed service hoặc self-hosted
- Backup: cron script upload S3

Monitoring:
- Prometheus + Grafana trên 1 VPS nhỏ
- Alertmanager + alert-sink trên 1 service
...
- Worker → Deployment (Keda auto-scale by queue length)
- Postgres → StatefulSet hoặc managed service
