...
| **worker** (Bull queue) | Background Jobs | Xử lý các tác vụ ngầm (Email/Reindex) không làm block Main Thread |
...
|---|---|---|---|
| **Postgres** (Database) | Source of Truth | Lưu trữ TẤT CẢ dữ liệu vĩnh viễn (Posts, Users, Comments) | Reliable, ACID compliance, Powerful Querying |
| **Redis** (Cache) | Distributed Cache | Lưu trữ tạm thời (Sessions, Recent posts) để tăng tốc độ | API Response nhanh gấp 10 lần |
| **Redis** (Queue) | Job Queue | Quản lý hàng chờ công việc (Email, Reindex) | Xử lý Async (Bất đồng bộ) mượt mà |
...
|---------|----------|---|
| Email (SMTP/SendGrid) | Email Delivery | Gửi Password Reset, Notifications từ Worker |
...
CMS lưu dữ liệu vào Postgres (Sync)
  ...
  ↓
CMS đẩy một Job vào Redis Queue: {type: 'reindex_post', postId: 123}
  ↓
Giao diện Admin trả về 200 OK ngay lập tức

Phía sau (Background), Worker:
  ↓
Lấy (Pull) Job từ Redis Queue
  ↓
...
|-----------|-------------------|-----------------|----------------|
| Postgres | Down toàn hệ thống | Restart service hoặc Restore Backup | 5-30 phút |
| Redis | App chạy chậm lại, Queue bị nghén | Restart service | < 1 phút |
...
| Caddy | Người dùng không thể truy cập | Restart service | < 1 phút |
| Worker | Tác vụ ngầm bị trì trệ | Restart Worker service | < 1 phút |
...
- [ ] Thử nghiệm Failover (Khởi động lại các Service xem có tự phục hồi không).
...
**Cache + Queue + Search + Monitoring = Fast, Reliable, Observable System**
