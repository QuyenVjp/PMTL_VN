# PMTL Infrastructure: Deep Dive

Giải thích sâu hơn **khi nào mới cần bật** các thành phần nặng hơn của hạ tầng.
Tài liệu này không phải checklist baseline cho phase đầu.
Nó cũng không phải nơi định nghĩa launch requirements.

> Chuẩn hóa mới của design:
> - baseline phase 1 ưu tiên `apps/web + apps/api + apps/admin + Postgres + Caddy + local storage + logs + health + backup/restore + security hardening`
> - `Valkey`, `BullMQ`, `outbox_events`, `apps/worker`, `Meilisearch`, `PgBouncer`, `Prometheus/Grafana/Alertmanager` không phải baseline mặc định; chỉ bật khi có nhu cầu thật
> - nếu business event đã được xác định là quan trọng và async, flow đúng là `Postgres transaction + outbox_events + dispatcher + execution queue`
> - không dùng Redis/Valkey pub/sub thuần làm handoff cho search/notify/email/webhook quan trọng khi async reliability đã được bật
> - `Meilisearch` chỉ là engine chính khi public search đã đủ core để warrant một search service riêng
> - `pgvector` chỉ là capability bổ sung khi recommendation / related-content đã được chốt
> - observability current phase nên lean: `Pino + /health/* + /metrics + runbook + restore drills`; metrics stack đầy đủ là optional phase sau
> - boundary chuẩn là `Zod validation + env contracts` cho request, queue payload, webhook payload và search document
> - current production fit cho media/file là `storage abstraction + local disk adapter`, còn `S3-compatible object storage` là target phase
> - preferred queue implementation là `BullMQ`; `Temporal` và `QStash` không phải baseline
> - preferred target object storage là `Cloudflare R2`
...
- pain thresholds để cân nhắc bật thêm component
- hậu quả vận hành khi đã bật chúng
...
- Nếu bạn chưa từng restore DB thành công, chưa có runbook thật, chưa có alert owner, thì không được bật thêm cụm hạ tầng nặng chỉ vì docs này mô tả nó.
- Nếu bạn chưa đo được pain bằng số liệu hoặc incident cụ thể, mặc định coi mọi component optional trong file này là `chưa nên bật`.
- File này để hiểu trước, không phải để copy đủ bộ.
...
- `Phase 1 baseline`: thứ phải có để ship, debug, backup, restore và secure nổi.
- `Phase 2 reliability`: chỉ bật khi async side effect hoặc search đã đủ đáng tiền.
- `Phase 3 observability/tuning`: chỉ bật khi anh thực sự biết metric hoặc symptom nào đang cần nhìn.
- Nếu chưa qua được `Phase 1 baseline`, dừng ở đó.

```
┌───────────────────────────────────────────────────────┐
│  BUSINESS LOGIC LAYER (App Layer)                     │
│  - pmtl-web (Frontend)                                │
│  - pmtl-api (NestJS Backend/API)                      │
│  - pmtl-admin (Custom Admin UI)                       │
│  - pmtl-worker (Background Jobs, optional phase 2)    │
└───────────────────────────────────────────────────────┘
                        |
         ┌──────────────┴──────────────┐
         |                             |
┌────────v────────┐        ┌──────────v──────────┐
│ DATA LAYER      │        │ ASYNC RELIABILITY LAYER (optional)   │
│ - Postgres      │        │ - outbox_events                      │
│ - Valkey        │        │ - dispatcher                         │
│ - Meilisearch   │        │ - Valkey/Redis-compatible execution queue │
│ - PgBouncer     │        │ - worker (tiến trình xử lý nền)      │
└─────────────────┘        └─────────────────────┘
          |
┌─────────v────────────────────────────────────┐
│ POOL & CONNECTION MGMT (optional)             │
...
│ - Alert Sink (custom handlers, optional)      │
└───────────────────────────────────────────────┘
```
...
```
App (API/worker (tiến trình xử lý nền))
    ↓
...
### 2. **pmtl-valkey** — Cache + execution queue (hàng đợi thực thi)

**Không bật phần này nếu**:
- app vẫn chạy ổn với 1 process đơn giản
- rate limit hiện tại chưa đòi hỏi shared state
- chưa có 1 side effect cụ thể nào đủ chậm hoặc đủ quan trọng để justify queue
...
```
User loads homepage
→ check Valkey recent_posts (hit)
→ return data (fast)

vs

→ check Valkey recent_posts (miss)
→ query Postgres
→ store in Valkey with TTL 1 hour
→ return data
```
...
```
User publish post
→ save canonical data to Postgres
...
Poll outbox_events
    ↓
Push job to Valkey/Redis-compatible execution queue (hàng đợi thực thi)
    ↓
Worker (tách biệt)
    ↓
Poll Valkey/Redis-compatible execution queue (hàng đợi thực thi)
    ↓
...
- execution queue lỗi → async (bất đồng bộ) job không được xử lý ngay, nhưng outbox event chưa mất
...
**Không bật phần này nếu**:
- chưa thấy connection pressure thật
- chưa hiểu transaction pooling và ORM interaction
...
**Không bật phần này nếu**:
- search chưa là feature core
- Postgres query vẫn chấp nhận được
- chưa có plan reindex, drift recovery, backup index
...
User publish post (Postgres)
    ↓
append outbox event `post.published`
    ↓
dispatcher phát execution job `{type: 'index_post', data: {...}}`
    ↓
...
| Execution queue length   | > 1000             | Add worker (tiến trình xử lý nền) instance                          |
...
| Redis         | Cache/execution queue lỗi          | Slow API, async (bất đồng bộ) job backlog             | Restart (< 1 min), replay từ outbox nếu cần            |
...
- Valkey managed service (lớp xử lý nghiệp vụ) hoặc self-hosted
- Local disk storage adapter cho media/file ở current phase
- Storage abstraction bắt buộc để chuẩn bị đường nâng cấp sang S3-compatible
...
- apps/api → Deployment (scale replicas)
- apps/admin → Deployment hoặc static hosting phía sau Caddy
- worker (tiến trình xử lý nền) → Deployment (Keda auto-scale by execution queue length)
- Postgres → StatefulSet hoặc managed service (lớp xử lý nghiệp vụ)
- Valkey → Deployment + PVC
...
Cho PMTL_VN hiện tại, nên ưu tiên:

- single VPS
- `BullMQ` cho Valkey/Redis-compatible execution queue
- local disk storage adapter
- metadata file canonical trong Postgres
- `Cloudflare` đứng trước `Caddy` nếu muốn CDN/SSL/basic edge protection
- `Cloudflare R2` là target storage phase sau
- outbox + execution queue + worker chỉ bật khi async side effect đủ đáng tiền
- app-layer rate limit là bắt buộc; shared counter bằng Valkey/Redis-compatible store bật khi flow đã cần
- `/health/live`, `/health/ready`, `/health/startup`, `/metrics`

Observability current phase:

- `Pino` + health endpoints + `/metrics` + runbook + restore drill
- chỉ self-host `Prometheus + Grafana + Alertmanager` khi đã có metric/alert use case rõ
- chỉ bật OTEL/tracing khi thật sự cần
- nếu cần traces sớm, cân nhắc managed backend trước khi tự host `Tempo`

Không cần nhảy lên object storage ngay nếu:

- adapter storage đã rõ
- object key an toàn
- backup local volume được chốt
- migration path sang `S3-compatible` đã được mô tả
...
- [ ] Storage abstraction + local adapter + migration path sang `S3-compatible`
...
- [ ] App-layer Redis rate limit rules cho auth/search/write/upload
- [ ] Health endpoints và `/metrics` đã chốt contract
- [ ] `audit_logs` và `feature_flags` đã có schema/helper
