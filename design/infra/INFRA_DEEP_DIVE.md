...
> - business event quan trọng đi theo `Postgres transaction + outbox_events + dispatcher + execution queue`
> - không dùng Redis/Valkey pub/sub thuần làm handoff cho search/notify/email/webhook quan trọng
> - `Meilisearch` vẫn là engine chính cho public search
> - `pgvector` chỉ là capability bổ sung khi recommendation / related-content đã được chốt
> - observability current phase nên lean: `Prometheus + Grafana + Pino`, còn traces là optional upgrade
...
└───────────────────────────────────────────────────────┘
         ...
         |                             |
┌────────v────────┐        ┌──────────v──────────┐
│ DATA LAYER      │        │ ASYNC RELIABILITY LAYER              │
...
### 2. **pmtl-redis** — Cache + execution queue (hàng đợi thực thi)
...
Poll outbox_events
    ↓
Push job to Redis execution queue (hàng đợi thực thi)
    ↓
Worker (tách biệt)
    ↓
Poll Redis execution queue (hàng đợi thực thi)
    ↓
...
- outbox + execution queue + worker
- app-layer rate limit bằng Valkey/Redis-compatible store
- `/health/live`, `/health/ready`, `/health/startup`, `/metrics`

Observability current phase:

- self-host `Prometheus + Grafana + Alertmanager + Pino`
