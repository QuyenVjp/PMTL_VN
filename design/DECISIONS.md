...
> **`pgvector` không phải deferred — là explicitly excluded.** Xem section 15 và `baseline/pgvector-decision.md`.
...
## 15. Deferred tech — explicit decisions with design-ready status (2026-03-21)

Các component dưới đây là "deferred" nhưng đã có FULL design doc.
Coding agent có thể activate ngay khi trigger được đáp ứng — không cần phát minh lại kiến trúc.
...
| Meilisearch | planned | SQL search p95 > 500ms | `06-search/meilisearch-architecture.md` |
...
| pgvector | **explicitly excluded** | Meilisearch stable 3+ months + semantic use case | `baseline/pgvector-decision.md` |
