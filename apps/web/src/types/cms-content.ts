...
/** Generic single item response from the CMS */
export interface CmsSingle<T> {
  data: T
  meta: Record<string, unknown>
}

/** CMS media/image format */
...
/** Shared UI icon entity from the CMS */
...
export type CmsApiResponse<T> = T | CmsApiError

export function isCmsApiError(res: unknown): res is CmsApiError {
  return (
    typeof res === 'object' &&
    res !== null &&
    'error' in res &&
    typeof (res as CmsApiError).error?.status === 'number'
  )
}
...
export interface BlogComment {
  ...
  documentId: string
  authorName: string
  authorAvatar: CmsMedia | string | null
  userId: string | null
  content: string
  likes: number
  createdAt: string
