      ...
      <main className="route-shell">
        <div className="route-frame">
          ...
          <div className="flex gap-2 flex-wrap justify-center mb-6">
            ...
            {regions.map((r) => (
              <button key={r.id} onClick={() => setActiveRegion(activeRegion === r.id ? null : r.id)} className={`filter-chip ${activeRegion === r.id ? "filter-chip-active" : "filter-chip-idle"}`}>{r.name}</button>
            ))}
          </div>

          <div className="max-w-lg mx-auto mb-10 relative">
            <SearchIcon className="absolute left-4 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
            <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Tìm theo tên hoặc địa chỉ..." className="toolbar-search py-3 pl-12 pr-4" />
          </div>
