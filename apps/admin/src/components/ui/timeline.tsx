 ...
 *
 * Composable sub-components: Timeline, TimelineItem, TimelineDot,
 * TimelineContent, TimelineHeading, TimelineLine.
 *
 * Ported from Shadboard, adapted for PMTL admin:
 * - Removed "use client" (Vite SPA)
 * - Replaced DynamicIcon with direct Lucide icon imports
 * - Uses PMTL design tokens
 */
...
> = {
  ...
  error: {
    icon: X,
    className:
      "bg-destructive text-destructive-foreground p-0.5 rounded-full",
  },
};
...
interface TimelineLineProps extends ComponentProps<typeof Separator> {
  done?: boolean;
}
