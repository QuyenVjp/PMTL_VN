  ...
  return ensurePublicId(
    {
      ...
      ...(input.content !== undefined ? { content, spamScore: scoreSpam(content) } : {}),
      ...(input.type !== undefined ? { type: sanitizeText(input.type) || "story" } : {}),
      ...(input.slug !== undefined ? { slug: sanitizeText(input.slug) } : {}),
      ...(input.videoURL !== undefined ? { videoURL: sanitizeText(input.videoURL) } : {}),
      ...(input.category !== undefined ? { category: sanitizeText(input.category) } : {}),
      ...(input.tags !== undefined ? { tags: normalizeTagItems(input.tags) } : {}),
      ...(input.rating !== undefined ? { rating: normalizeNumber(input.rating) } : {}),
      ...(input.authorNameSnapshot !== undefined ? { authorNameSnapshot: sanitizeText(input.authorNameSnapshot) } : {}),
      ...(input.likes !== undefined ? { likes: normalizeNumber(input.likes) } : {}),
      ...(input.views !== undefined ? { views: normalizeNumber(input.views) } : {}),
      ...(input.commentsCount !== undefined ? { commentsCount: normalizeNumber(input.commentsCount) } : {}),
      ...
      ...(input.reportCount !== undefined ? { reportCount: normalizeNumber(input.reportCount) } : {}),
      ...(input.lastReportReason !== undefined ? { lastReportReason: sanitizeText(input.lastReportReason) } : {}),
      ...(input.isHidden !== undefined ? { isHidden: Boolean(input.isHidden) } : {}),
    },
    "ugc",
  ) as T;
}
